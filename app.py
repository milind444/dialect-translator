...
# Load API key from Streamlit secrets when a secrets file is configured.
secrets_path = Path(__file__).parent / ".streamlit" / "secrets.toml"
if secrets_path.is_file() and "GROQ_API_KEY" in st.secrets:
...
with col2:
    ...
    if translate_btn:
        if not source_text.strip():
            ...
            st.error("Set your GROQ_API_KEY before translating.")
        else:
            result = None
            try:
                ...
                if "401" in str(error) or "invalid_api_key" in str(error):
                    st.error("The GROQ_API_KEY is invalid or expired. Create a new Groq API key and restart Streamlit.")
                else:
